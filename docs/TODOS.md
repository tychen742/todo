...
- Maps tab: separate top-level workspace tab with a New Map template picker, saved map tabs under MAPS, automatic local saving, direct node editing, draggable pill positions with explicit drag handles, selected-pill connector focus, a canvas control for top-level nodes, and child-node add controls on edge nodes.
