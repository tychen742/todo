...
- Workspace inbox: the assigned-to-me task inbox remains available inside Workspace as the personal side/inline panel; there is no separate Inbox tab.
- Maps tab: separate top-level workspace tab with a New Map template picker, saved map tabs under MAPS, synced personal maps in Supabase, automatic saving, direct node editing, draggable pill positions with explicit drag handles, selected-pill connector focus, adaptive connector curves, a larger stable map field for real planning space, stable saved starting positions, an essential inline inspector for layout/branch color/spacing, a canvas control for top-level nodes, and a selected-node action menu for child nodes, todo creation, and deletion.
...
- Current implementation: the personal workspace side/inline panel is the assigned-to-me task inbox; there is no separate Inbox tab. Maps is a separate Workspace tab with a template picker and saved map tabs under MAPS. New Map opens visual templates; choosing one creates an auto-saved Supabase row whose map nodes can be edited directly, selected, and dragged from explicit handles to manual positions, with adaptive connector curves, saved starting positions, top-level nodes added from the canvas, child nodes added from a selected-node action menu without shifting already materialized siblings, todo creation from the selected-node action menu, and a small inline inspector for Balanced/Right layout, colored branches, and compact spacing.
- Extend map storage beyond owner-only personal maps after the team/project workspace scope is settled.
...
- Phase columns can be renamed and deleted per project; deleting a phase moves its tasks back to Backlog. ✓ Done.
...
- Enforce 1-5 phase columns per project. ✓ Done.

## Project Management

- Keep todos created in the personal Workspace visible there after a project is attached; they also appear in the project's Backlog until moved into a phase. ✓ Done.
