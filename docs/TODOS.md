...
- Maps tab: separate top-level workspace tab with a New Map template picker, an on-page saved map list, automatic local saving, direct node editing, draggable pill positions, a canvas control for top-level nodes, and child-node add controls on edge nodes.
