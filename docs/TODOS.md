...
- Maps tab: separate top-level workspace tab with a New Map template picker, saved map tabs under MAPS, automatic local saving, direct node editing, draggable pill positions with explicit drag handles, selected-pill connector focus, adaptive connector curves, a visible stable map field, stable saved starting positions, an essential inline inspector for layout/branch color/spacing, a canvas control for top-level nodes, and a selected-node action menu for child nodes, todo creation, and deletion.
