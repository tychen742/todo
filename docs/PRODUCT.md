...
- The account header should show presence compactly: the personal status message appears inline after the username, the caret opens account/navigation controls, the green dot means the browser is actively focused, and the small activity bar represents an 8-hour work window. Time actively displaying the Workspace tab fills the bar in green.
