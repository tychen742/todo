...
- The header should reserve a compact message board beside search on desktop. It should use distinct left-bar colors for browser/presence activity and personal chat or status messages, starting with lightweight items such as "Alice is online" and "Alice: I am up to something." The header date/time is hidden temporarily; search should use the Inbox column width and align with the right-side Inbox pane.
- Presence and activity indicators need user education in onboarding/help, with copy maintained in `docs/COMMUNICATIONS.md`.
...
- Quick capture should support project routing through attribute tokens in the todo text. Legacy leading abbreviation prefixes such as `re: do something` still work. Abbreviation tokens with `:` or `+`, such as `:re do something`, `+re do something`, or `do something +re`, should also create `do something` in the Research project. Priority tokens should use `:U` urgent, `:H`, `:HI`, or `:HIGH` high, `:N`, `:M`, `:NORMAL`, or `:MEDIUM` normal, and `:L`, `:LO`, or `:LOW` low. Attribute tokens may appear at the beginning or end of the input and are stripped from saved task text. If two or more projects share the same abbreviation, the abbreviation is ambiguous and the user must type the full project name instead.
- Quick capture may chain project and priority tokens at the start of a task, for example `:li :hi higheredjobs every saturday`.
...
- Maps should be a separate workspace-level concept. Current UI gives Maps a primary workspace tab where New Map opens visual templates, created maps are saved as tabs under MAPS, personal maps sync through Supabase, maps save automatically, nodes are edited directly, pills can be selected and dragged from explicit handles to manual positions, top-level nodes can be added from the canvas, selected pills expose a compact action menu for child nodes, todo creation, and deletion, individual nodes can create regular todos through the existing quick-capture path, selected connector paths are visually emphasized, connector curves adapt to each node's relative position, adding branches preserves already materialized node positions, the map canvas gives planning work generous space while keeping a stable grid field, and an essential inline inspector supports layout, colored branches, and compact spacing only.
- Maps should eventually connect directly to project planning: a map can belong to a project, appear from that project, and act as a visual plan for that bounded effort. Individual map nodes should be able to link to existing todos or create linked todos, so visual structure and execution status stay connected without duplicating task systems.
- Maps should avoid becoming a heavy diagram editor. Defer stickers, illustrations, equations, attachments, deep typography, and detailed shape/border controls until they clearly support project work.
- Maps marketing language is tracked in `docs/MARKETING.md`; preserve the "thinking on paper with structure" positioning as the feature matures.
