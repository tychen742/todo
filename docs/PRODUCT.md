...
- Maps should be a separate workspace-level concept. Current UI gives Maps a primary workspace tab where New Map opens visual templates, created maps are saved as tabs under MAPS, maps save automatically, nodes are edited directly, pills can be dragged to manual positions, top-level nodes can be added from the canvas, and edge nodes expose child-node add controls.
