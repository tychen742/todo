...
Auth form labels use `marginBottom: 6`. Auth form text inputs, password wrappers, and submit buttons use `height: 48`. The email input uses `marginBottom: 12`; the password wrapper keeps `marginBottom: 30`, while the auth submit button has no top margin. The auth panel uses `maxWidth: 360` so the form stays compact on desktop web.
