...
Superseded UI decision: Inbox was available as a primary Workspace tab, while Maps owns the visual planning surface. Current UI keeps assignment inboxes inside Workspace as the personal side/inline panel, and assignment notifications should move to a lighter notification surface instead of competing with the main workspace.
...
truncated task names are still inspectable without opening the todo.
...
Auth form labels use `marginBottom: 6`. Auth form text inputs, password wrappers, and submit buttons use `height: 48`. The email input uses `marginBottom: 12`; the password wrapper keeps `marginBottom: 16`; the auth links row uses `marginTop: 6`, while the auth submit button has no top margin. The auth panel uses `maxWidth: 360` so the form stays compact on desktop web.

Auth input text uses `14px`, and the password visibility toggle uses `13px`, so placeholder text and `Show`/`Hide` controls stay visually subordinate to the form labels and actions.
...
Quick-capture routing uses attribute tokens in the add-todo text. Legacy leading project prefixes such as `re:` for Research or `te:` for Teaching still work. Project abbreviations can also be written as `:re` or `+re` at the beginning or end of the input, and unambiguous single-word project-name tokens are accepted. Priority can be set with `:U` urgent, `:H` high, `:N` or `:M` normal, and `:L` low. Recognized tokens are removed from the saved task text. If multiple projects share the same abbreviation, the app must not guess; it requires the full project name before assigning the project. Unknown tokens are treated as ordinary task text so normal colon usage does not block capture.
